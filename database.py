...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Voice analysis results table (NEW)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS voice_analysis_results (
                    ...
                    filler_word_percentage REAL DEFAULT 0.0,
                    words_per_minute REAL DEFAULT 0.0,
                    total_words INTEGER DEFAULT 0,
                    speaking_duration_seconds REAL DEFAULT 0.0,
            ...
            # Mock viva sessions table (NEW)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS mock_viva_sessions (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    user_id INTEGER NOT NULL,
            ...
            # Mock viva answers table (NEW)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS mock_viva_answers (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    time_taken INTEGER,
                    answered_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (session_id) REFERENCES mock_viva_sessions (id)
                )
            """)
            
            # Study activity log for heatmap and patterns (NEW)
            cursor.execute("""
            ...
            # User achievements/badges table (NEW)
            cursor.execute("""
    ...
    # =============================================
    # NEW: Voice Analysis Methods
    # =============================================
    
    def save_voice_analysis(self, user_id: int, transcript: str, analysis_result: dict, 
                           answer_id: int = None) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    (user_id, answer_id, transcript, filler_word_count, filler_words_json,
                     filler_word_percentage, words_per_minute, total_words, 
                     speaking_duration_seconds, confidence_score, overall_score,
                     strengths_json, improvements_json)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    user_id, answer_id, transcript,
                    analysis_result.get('filler_word_count', 0),
                    json.dumps(analysis_result.get('filler_words_found', {})),
                    analysis_result.get('filler_word_percentage', 0),
                    analysis_result.get('words_per_minute', 0),
                    analysis_result.get('total_words', 0),
                    analysis_result.get('speaking_duration_seconds', 0),
                    analysis_result.get('confidence_score', 0),
                    analysis_result.get('overall_score', 0),
                    json.dumps(analysis_result.get('strengths', [])),
                    json.dumps(analysis_result.get('areas_for_improvement', []))
                ))
                
                conn.commit()
                return True
        except Exception as e:
            print(f"Error saving voice analysis: {str(e)}")
            return False
    
    def get_user_voice_analytics(self, user_id: int, limit: int = 50) -> List[Dict]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    SELECT filler_word_count, filler_word_percentage, words_per_minute,
                           confidence_score, overall_score, created_at
                ...
                for row in cursor.fetchall():
                    results.append({
                        'filler_word_count': row[0],
                        'filler_word_percentage': row[1],
                        'words_per_minute': row[2],
            ...
            print(f"Error getting voice analytics: {str(e)}")
            return []
    
    # =============================================
    # NEW: Mock Viva Session Methods
    # =============================================
    
    def create_mock_viva_session(self, user_id: int, interviewer_persona: str, 
                                  interviewer_name: str, topic: str, 
                                  total_questions: int) -> int:
        """Create a new mock viva session"""
        try:
            import uuid
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    INSERT INTO mock_viva_sessions 
                    (user_id, session_token, interviewer_persona, interviewer_name,
                     topic, total_questions, max_possible_score)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (user_id, session_token, interviewer_persona, interviewer_name,
                      topic, total_questions, total_questions * 10))
                
                conn.commit()
                return cursor.lastrowid
        except Exception as e:
            print(f"Error creating mock viva session: {str(e)}")
            return None
    
    def update_mock_viva_session(self, session_id: int, updates: dict) -> bool:
        """Update mock viva session"""
        try:
            import json
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                for key, value in updates.items():
                    if key == 'conversation_history':
                        set_clauses.append("conversation_history_json = ?")
            ...
            print(f"Error updating mock viva session: {str(e)}")
            return False
    
    def save_mock_viva_answer(self, session_id: int, question_text: str, 
                              correct_answer: str, user_answer: str, score: int,
                              feedback: str = None, follow_up_question: str = None,
                              follow_up_answer: str = None, time_taken: int = None) -> bool:
        """Save an answer in mock viva session"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                
                cursor.execute("""
                    INSERT INTO mock_viva_answers 
                    (session_id, question_text, correct_answer, user_answer, score,
                     feedback, follow_up_question, follow_up_answer, time_taken)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (session_id, question_text, correct_answer, user_answer, score,
                      feedback, follow_up_question, follow_up_answer, time_taken))
            ...
            print(f"Error saving mock viva answer: {str(e)}")
            return False
    
    def get_user_mock_viva_history(self, user_id: int, limit: int = 20) -> List[Dict]:
        """Get user's mock viva session history"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                
                cursor.execute("""
                    SELECT id, interviewer_persona, interviewer_name, topic,
                           total_questions, questions_answered, total_score,
                           max_possible_score, follow_up_count, hints_used,
                           status, started_at, ended_at, duration_seconds,
                           performance_rating
                    FROM mock_viva_sessions
                    WHERE user_id = ?
                ...
                for row in cursor.fetchall():
                    sessions.append({
                        ...
                        'performance_rating': row[14]
                    })
                
                return sessions
        except Exception as e:
            print(f"Error getting mock viva history: {str(e)}")
            return []
    
    # =============================================
    # NEW: Study Activity & Analytics Methods
    # =============================================
    
    def log_study_activity(self, user_id: int, activity_type: str, subject: str = None,
                          topic: str = None, questions_attempted: int = 0,
                          questions_correct: int = 0, total_score: int = 0,
                          max_score: int = 0, duration_seconds: int = 0,
                          session_id: int = None) -> bool:
        """Log study activity for analytics and heatmap"""
        try:
            from datetime import datetime
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    INSERT INTO study_activity_log 
                    (user_id, activity_type, session_id, subject, topic,
                     questions_attempted, questions_correct, total_score, max_score,
                     duration_seconds, activity_date, activity_hour)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (user_id, activity_type, session_id, subject, topic,
                      questions_attempted, questions_correct, total_score, max_score,
                      duration_seconds, now.date(), now.hour))
                
                conn.commit()
                return True
        except Exception as e:
            print(f"Error logging study activity: {str(e)}")
            return False
    
    def get_user_activity_data(self, user_id: int, days: int = 365) -> List[Dict]:
        """Get user activity data for heatmap and analytics"""
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    SELECT activity_type, subject, topic, questions_attempted,
                           questions_correct, total_score, max_score, duration_seconds,
                           activity_date, activity_hour, created_at
    ...
    def get_user_performance_data(self, user_id: int) -> List[Dict]:
        """Get comprehensive performance data for analytics dashboard"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                
                # Get all answers from regular sessions
                cursor.execute("""
                    SELECT ua.score, ua.answered_at, ua.time_taken,
                           q.difficulty_level, c.subject
                    FROM user_answers ua
                    JOIN questions q ON ua.question_id = q.id
                ...
                for row in cursor.fetchall():
                    performance_data.append({
                        ...
                        'difficulty': row[3],
                        'subject': row[4],
                        'topic': row[4]  # Using subject as topic for now
                    })
                
                # Also get predefined question answers
                cursor.execute("""
                    SELECT pqa.score, pqa.answered_at, pqa.time_taken,
                           qb.difficulty, s.name as subject, t.name as topic
                    FROM predefined_question_answers pqa
                    JOIN question_bank qb ON pqa.question_id = qb.id
                    JOIN predefined_question_sessions pqs ON pqa.session_id = pqs.id
                    JOIN subjects s ON qb.subject_id = s.id
                    LEFT JOIN topics t ON qb.topic_id = t.id
                    WHERE pqs.user_id = ?
    ...
    def award_achievement(self, user_id: int, achievement_type: str, 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                return cursor.rowcount > 0  # True if new achievement was inserted
        except Exception as e:
